...
static void simplifyDP(float tol, ofxVec2f* v, int j, int k, int* mk ){
    ...
    ofxVec2f  w;
    ofxVec2f   Pb;                // base of perpendicular from v[i] to S
...
static vector <ofxVec2f> simplifyContour(vector <ofxVec2f> &V, float tol) {
	
	int n = V.size();
	vector <ofxVec2f> sV;
	if(n <= 2) return sV;
	sV.assign(n, ofxVec2f());
	
    int    i, k, m, pv;            // misc counters
    float  tol2 = tol * tol;       // tolerance squared
    ofxVec2f * vt = new ofxVec2f[n];
	int * mk = new int[n];
...
//-------------------------------------------------------------------
static bool insidePolygon(const ofxVec2f & p, const vector<ofxVec2f> & polygon){
...
//-------------------------------------------------------------------
static ofxVec2f getTriangleCenter(const ofxVec2f & a, const ofxVec2f & b, const ofxVec2f & c) {
	ofxVec2f center;
	center += a;
	center += b;
	center += c;
...
//-------------------------------------------------------------------
static ofRectangle getPolygonBounds(const vector <ofxVec2f> & vertices) {
	ofRectangle bounds;
	bounds.x        = 100000;
	bounds.y	    = 100000;
	bounds.width    = 0;
...
//-------------------------------------------------------------------
static void addRandomPointsInside(vector <ofxVec2f> & vertices, int amt=100) {
	...
	for (int i=0; i<amt; i++) {
		...
		if(insidePolygon(ofxVec2f(x, y), vertices)) vertices.push_back(ofxVec2f(x, y));
	}
}


//-------------------------------------------------------------------
static vector <ofxVec2f> triangulatePolygon(const vector <ofxVec2f> &ptsIn, bool addPointsInside=false, int amt=100) {
	
	vector <ofxVec2f> pts;
	vector <ofxVec2f> triangles;
	...
	for(Delaunay::fIterator fit=delobject.fbegin(); fit!=delobject.fend(); ++fit) {
		...
		ofxVec2f center = getTriangleCenter(a, b, c);
