...
//----------------------------------------
ofxVec2f ofxBox2dPolygon::getCenter() {	
	if(!body) return 0;
	const b2Transform& xf = body->GetTransform();
	b2Vec2 b2Center = b2Mul(xf, b2Vec2(center.x/OFX_BOX2D_SCALE, center.y/OFX_BOX2D_SCALE));
	return ofxVec2f(b2Center.x*OFX_BOX2D_SCALE, b2Center.y*OFX_BOX2D_SCALE);
};
...
//----------------------------------------
void ofxBox2dPolygon::addVertex(const ofxVec2f & p) {
	vertices.push_back(p);
}

//----------------------------------------
void ofxBox2dPolygon::addVertex(float x, float y) {
	addVertex(ofxVec2f(x, y));
}

//----------------------------------------
void ofxBox2dPolygon::addVertexes(const vector <ofxVec2f> & polyPoints) {
...
void ofxBox2dPolygon::create(b2World * b2dworld) {
	...
	for(Delaunay::fIterator fit=delobject.fbegin(); fit!=delobject.fend(); ++fit) {
		...
		ofxVec2f a = vertices[pta];
		ofxVec2f b = vertices[ptb];
		ofxVec2f c = vertices[ptc];
